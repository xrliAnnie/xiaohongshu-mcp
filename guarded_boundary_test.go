...
func TestGuardedBoundaryRequiresSignedMatchingMeasurements(t *testing.T) {
	...
	statement := guardedBoundaryStatement{SchemaVersion: 1, ConfigDigest: config, ProviderBinarySHA256: binary, ToolSchemaDigest: schema, Passed: true}
	...
	if verifyGuardedBoundary(raw, pub, config, binary, schema) != nil {
	...
	for _, m := range [][3]string{{strings.Repeat("d", 64), binary, schema}, {config, strings.Repeat("d", 64), schema}, {config, binary, strings.Repeat("d", 64)}} {
		if verifyGuardedBoundary(raw, pub, m[0], m[1], m[2]) == nil {
...
func TestGuardedBoundaryRejectsMissingReceipt(t *testing.T) {
	if verifyGuardedBoundary(nil, make([]byte, 32), strings.Repeat("a", 64), strings.Repeat("b", 64), strings.Repeat("c", 64)) == nil {
